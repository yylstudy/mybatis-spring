...
/**
 * mapper扫描配置器，实现了BeanDefinitionRegistryPostProcessor接口和InitializingBean接口
 * BeanDefinitionRegistryPostProcessor ，这个接口用于在初始化MapperScannerConfigurer的时候，
 * 会执行其postProcessBeanDefinitionRegistry方法，这个和Spring初始化Configuation注解的时候是一样的
 * 有个ConfigurationClassPostProcessor这个类
 */
public class MapperScannerConfigurer implements BeanDefinitionRegistryPostProcessor, InitializingBean, ApplicationContextAware, BeanNameAware {
  ...
  /**
   * 这个是spring 容器启动的时候refresh方法中invokeBeanFactoryPostProcessors方法会调用这个方法
   * {@inheritDoc}
   * 
   * @since 1.0.2
   */
